...
<p class="id">Sebuah dompet Ethereum kehilangan 999.999 USDT akibat satu tanda tangan persetujuan token berbahaya pada 9 Juli, menurut Cointelegraph, CryptoTimes, dan crypto.news. Skrip penguras otomatis milik penyerang awalnya mencoba menarik seluruh saldo korban dalam satu transaksi, yang gagal karena dana tidak mencukupi, lalu menghitung ulang 36 detik kemudian dan mengeksekusi tiga penarikan terpisah sebesar 639.999, 159.999, dan 200.000 USDT yang digabungkan melalui satu transaksi Multicall.</p>
<p class="pl">Portfel Ethereum stracił 999 999 USDT w wyniku jednego złośliwego podpisu zatwierdzenia tokena 9 lipca, jak podały Cointelegraph, CryptoTimes i crypto.news. Zautomatyzowany skrypt drenujący atakującego najpierw próbował wypłacić całe saldo ofiary w jednej transakcji, co nie powiodło się z powodu niewystarczających środków, następnie przeliczył ponownie 36 sekund później i wykonał trzy oddzielne wypłaty w wysokości 639 999, 159 999 i 200 000 USDT połączone w jedną transakcję Multicall.</p>
<p class="it">Un portafoglio Ethereum ha perso 999.999 USDT a causa di una singola firma malevola di approvazione token il 9 luglio, secondo Cointelegraph, CryptoTimes e crypto.news. Lo script di prosciugamento automatizzato dell'attaccante ha prima tentato di prelevare l'intero saldo della vittima in un'unica transazione, fallita per fondi insufficienti, poi ha ricalcolato 36 secondi dopo ed eseguito tre prelievi separati di 639.999, 159.999 e 200.000 USDT raggruppati in un'unica transazione Multicall.</p>
<p class="ru">Кошелёк Ethereum потерял 999 999 USDT из-за одной вредоносной подписи одобрения токена 9 июля, сообщают Cointelegraph, CryptoTimes и crypto.news. Автоматизированный скрипт злоумышленника сначала попытался вывести весь баланс жертвы одной транзакцией, что не удалось из-за нехватки средств, затем через 36 секунд пересчитал сумму и выполнил три отдельных вывода на 639 999, 159 999 и 200 000 USDT, объединённых в одну транзакцию Multicall.</p>
<p class="zh">根據Cointelegraph、CryptoTimes與crypto.news報導，一個以太坊錢包於7月9日因單次惡意代幣核准(approval)簽名損失了999,999 USDT。攻擊者的自動抽乾腳本一開始試圖在單筆交易中提走受害者的全部餘額，但因餘額不足而失敗，36秒後重新計算金額，並透過單一Multicall交易執行了三筆分別為639,999、159,999與200,000 USDT的提領。</p>
...
<h2 class="id">Anatomi Satu Tanda Tangan</h2>
<h2 class="pl">Anatomia Jednego Podpisu</h2>
<h2 class="it">Anatomia di una Singola Firma</h2>
<h2 class="ru">Анатомия Одной Подписи</h2>
<h2 class="zh">單一簽名的解剖</h2>
...
<p class="id">Korban kemungkinan besar berinteraksi dengan antarmuka palsu yang meniru layanan sah, atau dApp berbahaya, dan menandatangani permintaan yang disamarkan sebagai sesuatu yang rutin seperti "konfirmasi login." Tanda tangan persetujuan itu sendiri tidak membebankan biaya gas kepada korban dan tidak langsung disiarkan on-chain, sehingga tampaknya tidak terjadi apa-apa segera setelah menandatangani — pencurian sebenarnya terjadi kemudian, kapan pun kontrak penyerang menggunakan tanda tangan itu untuk memanggil transferFrom.</p>
<p class="pl">Ofiara najprawdopodobniej wchodziła w interakcję z fałszywym interfejsem naśladującym legalną usługę lub złośliwą dApp i podpisała żądanie zamaskowane jako coś rutynowego, na przykład „potwierdzenie logowania". Sam podpis zatwierdzenia nie kosztuje ofiary gazu i nie jest natychmiast transmitowany on-chain, więc bezpośrednio po podpisaniu wydaje się, że nic się nie dzieje — właściwa kradzież następuje później, gdy kontrakt atakującego użyje tego podpisu, aby wywołać transferFrom.</p>
<p class="it">La vittima molto probabilmente ha interagito con un'interfaccia falsa che imitava un servizio legittimo, o una dApp malevola, e ha firmato una richiesta camuffata da qualcosa di routine come una "conferma di accesso". La firma di approvazione in sé non costa gas alla vittima e non viene trasmessa immediatamente on-chain, quindi subito dopo la firma sembra che non accada nulla — il furto vero e proprio avviene più tardi, quando il contratto dell'attaccante usa quella firma per chiamare transferFrom.</p>
<p class="ru">Жертва, скорее всего, взаимодействовала с поддельным интерфейсом, имитирующим легитимный сервис, или вредоносным dApp, и подписала запрос, замаскированный под что-то рутинное, вроде «подтверждения входа». Сама подпись одобрения не стоит жертве газа и не транслируется в блокчейн немедленно, поэтому сразу после подписания кажется, что ничего не произошло — фактическая кража происходит позже, когда контракт злоумышленника использует эту подпись для вызова transferFrom.</p>
<p class="zh">受害者極有可能是在與模仿正規服務的假介面或惡意dApp互動時，簽署了偽裝成「登入確認」之類例行操作的請求。核准簽名本身不需要受害者支付Gas費，也不會立即被廣播上鏈，因此簽署後看起來像是「什麼都沒發生」——實際竊取發生在之後，也就是攻擊者的合約使用該簽名呼叫transferFrom的那一刻。</p>
...
<p class="id">Mekanisme spesifik yang terungkap di sini patut diperhatikan. Menggabungkan tiga penarikan terpisah ke dalam satu Multicall menunjukkan bahwa penyerang menyusun penguras dana untuk menghindari peringatan ukuran transaksi tunggal yang digunakan beberapa dompet dan bursa. Dan perhitungan ulang dalam 36 detik menunjukkan bahwa bot penyapu ini kini cukup otomatis untuk menyelidiki dan menyesuaikan hampir secara real time tanpa memerlukan campur tangan manual dari penyerang — upaya pertama yang gagal bukanlah pertahanan yang berhasil menahan, melainkan hanya data untuk upaya kedua.</p>
<p class="pl">Warto zwrócić uwagę na konkretne mechanizmy ujawnione w tym przypadku. Połączenie trzech oddzielnych wypłat w jeden Multicall sugeruje, że atakujący ustrukturyzował drenaż tak, aby ominąć alerty dotyczące rozmiaru pojedynczej transakcji, których używają niektóre portfele i giełdy. A ponowne przeliczenie po 36 sekundach pokazuje, że te boty czyszczące są obecnie na tyle zautomatyzowane, że mogą sondować i dostosowywać się niemal w czasie rzeczywistym bez konieczności ręcznej interwencji atakującego — pierwsza nieudana próba nie była obroną, która się utrzymała, była jedynie danymi dla drugiej.</p>
<p class="it">I meccanismi specifici rivelati qui meritano attenzione. Raggruppare tre prelievi separati in un unico Multicall suggerisce che l'attaccante abbia strutturato il prosciugamento per aggirare gli avvisi sulla dimensione delle transazioni singole utilizzati da alcuni portafogli ed exchange. E il ricalcolo in 36 secondi mostra che questi bot di prosciugamento sono ormai abbastanza automatizzati da sondare e adattarsi quasi in tempo reale senza richiedere l'intervento manuale dell'attaccante — il primo tentativo fallito non è stata una difesa che ha retto, era solo un dato per il secondo.</p>
<p class="ru">Конкретные механизмы, раскрытые здесь, заслуживают внимания. Объединение трёх отдельных выводов в один Multicall предполагает, что злоумышленник построил вывод средств так, чтобы обойти оповещения о размере отдельной транзакции, которые используют некоторые кошельки и биржи. А пересчёт через 36 секунд показывает, что эти боты-«выметатели» теперь достаточно автоматизированы, чтобы зондировать и корректировать действия почти в реальном времени без ручного вмешательства злоумышленника — первая неудачная попытка была не защитой, которая выстояла, а просто данными для второй.</p>
<p class="zh">此次事件揭露的具體手法值得關注。將三筆分開的提領合併成單一Multicall，顯示攻擊者刻意設計此次抽乾行動，以繞過部分錢包與交易所所使用的單筆交易規模警示。而36秒內完成的重新計算，顯示這類清空型機器人如今已高度自動化，能在近乎即時的情況下偵測並調整，無需攻擊者手動介入——第一次失敗的嘗試並非撐住的防線，只是為第二次嘗試提供的資料而已。</p>
...
<h2 class="id">Mengapa Persetujuan, Bukan Peretasan, Menjadi Mode Kegagalan yang Berulang</h2>
<h2 class="pl">Dlaczego Zatwierdzenia, a Nie Włamania, Są Powtarzającym Się Trybem Awarii</h2>
<h2 class="it">Perché le Approvazioni, Non gli Attacchi Hacker, Sono la Modalità di Fallimento Ricorrente</h2>
<h2 class="ru">Почему Одобрения, а Не Взломы, Являются Повторяющейся Причиной Сбоев</h2>
<h2 class="zh">為何「核准」而非「駭客攻擊」才是反覆出現的失敗模式</h2>
...
<p class="id">Mekanisme mendasar yang diungkap insiden ini adalah fungsi approve tanpa batas ERC-20 dan tanda tangan permit EIP-2612. Dalam standar Ethereum, fungsi approve mendelegasikan izin bagi alamat tertentu untuk memindahkan token Anda hingga jumlah yang ditentukan, tetapi banyak dApp meminta jumlah yang secara efektif tidak terbatas demi kenyamanan, dan pengguna secara terbiasa menyetujuinya. Permit EIP-2612 melangkah lebih jauh, memungkinkan delegasi itu terjadi melalui satu tanda tangan off-chain alih-alih transaksi on-chain, sehingga menandatangani tidak membutuhkan gas dan tidak memicu peringatan "transaksi tertunda" pada dompet. Ini bukan "peretasan" yang mengeksploitasi bug — ini adalah transfer persis seperti yang diotorisasi pengguna, itulah sebabnya tidak ada protokol yang disusupi dan tidak ada bug bounty yang berlaku.</p>
<p class="pl">Ukryty mechanizm, który ujawnia ten incydent, to nieograniczona funkcja approve ERC-20 oraz podpisy permit EIP-2612. W standardzie Ethereum funkcja approve deleguje danemu adresowi uprawnienie do przenoszenia Twoich tokenów do określonej kwoty, ale wiele dApp dla wygody żąda praktycznie nieograniczonych kwot, a użytkownicy nawykowo je zatwierdzają. Permit EIP-2612 idzie o krok dalej, pozwalając, aby ta delegacja odbyła się poprzez pojedynczy podpis off-chain zamiast transakcji on-chain, więc podpisanie nie kosztuje gazu i nie wywołuje w portfelu ostrzeżenia „transakcja oczekująca". To nie jest „włamanie" wykorzystujące błąd — to dokładnie ten transfer, który autoryzował użytkownik, dlatego żaden protokół nie został naruszony i nie ma zastosowania żadna nagroda za błędy (bug bounty).</p>
<p class="it">Il meccanismo sottostante che questo incidente espone è la funzione approve illimitata di ERC-20 e le firme permit di EIP-2612. Nello standard Ethereum, la funzione approve delega a un determinato indirizzo il permesso di spostare fino a un importo specificato dei tuoi token, ma molte dApp richiedono per comodità importi effettivamente illimitati, e gli utenti li approvano abitualmente. Il permit di EIP-2612 va oltre, permettendo che questa delega avvenga tramite una singola firma off-chain invece di una transazione on-chain, quindi firmare non costa gas e non attiva l'avviso "transazione in sospeso" del portafoglio. Questo non è un "hack" che sfrutta un bug — è esattamente il trasferimento che l'utente ha autorizzato, motivo per cui nessun protocollo è stato compromesso e nessuna bug bounty si applica.</p>
<p class="ru">Лежащий в основе этого инцидента механизм — это неограниченная функция approve стандарта ERC-20 и подписи permit EIP-2612. В стандарте Ethereum функция approve делегирует определённому адресу разрешение перемещать ваши токены до указанной суммы, но многие dApp ради удобства запрашивают фактически неограниченные суммы, а пользователи по привычке их одобряют. Permit EIP-2612 идёт ещё дальше, позволяя этой делегации происходить через единственную оффчейн-подпись вместо ончейн-транзакции, поэтому подписание не стоит газа и не вызывает предупреждение кошелька о «ожидающей транзакции». Это не «взлом», эксплуатирующий баг, — это именно тот перевод, который авторизовал пользователь, поэтому ни один протокол не был скомпрометирован и никакая программа bug bounty не применяется.</p>
<p class="zh">此事件揭露的根本機制，是ERC-20的無限額核准(approve)功能與EIP-2612的permit簽名。在以太坊標準中，approve函式會將特定地址移動您代幣至指定金額的權限委託出去，但許多dApp為了方便，實際上要求的是幾乎無限的額度，使用者也習慣性地予以核准。EIP-2612的permit更進一步，讓這種委託透過單一鏈下簽名而非鏈上交易完成，因此簽署不需支付Gas費，也不會觸發錢包的「交易待處理」警示。這並非利用漏洞的「駭客攻擊」——而正是使用者所授權的那筆轉帳本身，這也是為何沒有任何協議遭到入侵，也不適用任何錯誤賞金(bug bounty)。</p>
...
<p class="id">Yang mengindustrialisasi taktik ini adalah gelombang kit "drainer-as-a-service" seperti Inferno Drainer dan Pink Drainer yang telah menjamur sejak 2023. Menurut laporan 2025 ScamSniffer, total kerugian akibat phishing turun 83% menjadi sekitar $84 juta, dari sekitar $494 juta pada 2024, namun kerugian rata-rata per korban justru meningkat pada periode yang sama — penyerang telah beralih dari spam massal bernilai rendah menuju "perburuan paus" yang menargetkan secara presisi seperti kasus ini. Beberapa peneliti keamanan membingkai insiden seperti ini sebagai kesalahan pengguna, bukan kegagalan protokol, dengan berargumen bahwa mencabut persetujuan lama secara berkala atau menggunakan dompet perangkat keras dengan clear-signing akan mencegahnya. Pandangan itu bisa dibilang meremehkan betapa meyakinkannya antarmuka phishing modern kini meniru antarmuka yang sah.</p>
<p class="pl">Tym, co uprzemysłowiło ten schemat, była fala zestawów „drainer-as-a-service", takich jak Inferno Drainer i Pink Drainer, które rozpowszechniły się od 2023 roku. Według raportu ScamSniffer za 2025 rok, całkowite straty z phishingu spadły o 83% do około 84 milionów dolarów, z około 494 milionów dolarów w 2024 roku, jednak średnia strata na ofiarę wzrosła w tym samym okresie — atakujący przeszli od masowego, niskowartościowego spamu do precyzyjnie ukierunkowanego „polowania na wieloryby", takiego jak to. Niektórzy badacze bezpieczeństwa przedstawiają takie incydenty jako błąd użytkownika, a nie awarię protokołu, argumentując, że okresowe odwoływanie nieaktualnych zatwierdzeń lub użycie portfela sprzętowego z clear-signing zapobiegłoby temu. Ten pogląd prawdopodobnie nie docenia, jak przekonująco współczesne interfejsy phishingowe naśladują dziś legalne.</p>
<p class="it">A industrializzare questo schema è stata un'ondata di kit "drainer-as-a-service" come Inferno Drainer e Pink Drainer, proliferati dal 2023. Secondo il rapporto 2025 di ScamSniffer, le perdite totali da phishing sono scese dell'83% a circa 84 milioni di dollari, rispetto a circa 494 milioni nel 2024, ma la perdita media per vittima è aumentata nello stesso periodo — gli attaccanti si sono spostati dallo spam di massa a basso valore verso una "caccia alle balene" mirata con precisione come questa. Alcuni ricercatori di sicurezza inquadrano incidenti come questo come errore dell'utente piuttosto che fallimento del protocollo, sostenendo che revocare periodicamente le approvazioni obsolete o usare un portafoglio hardware con clear-signing lo avrebbe impedito. Questa visione probabilmente sottostima quanto le moderne interfacce di phishing imitino oggi in modo convincente quelle legittime.</p>
<p class="ru">Индустриализировала эту схему волна наборов «drainer-as-a-service», таких как Inferno Drainer и Pink Drainer, распространившихся с 2023 года. Согласно отчёту ScamSniffer за 2025 год, общие потери от фишинга упали на 83% до примерно 84 миллионов долларов по сравнению с примерно 494 миллионами долларов в 2024 году, однако средний убыток на жертву за тот же период вырос — злоумышленники перешли от массового низкоценного спама к точечной «охоте на китов», как в этом случае. Некоторые исследователи безопасности рассматривают подобные инциденты скорее как ошибку пользователя, чем как сбой протокола, утверждая, что периодический отзыв устаревших одобрений или использование аппаратного кошелька с clear-signing предотвратили бы это. Такая точка зрения, вероятно, недооценивает, насколько убедительно современные фишинговые интерфейсы теперь имитируют легитимные.</p>
<p class="zh">真正將這套手法產業化的，是自2023年起大量擴散的「抽乾即服務」(drainer-as-a-service)工具包，例如Inferno Drainer與Pink Drainer。根據ScamSniffer的2025年報告，網路釣魚總損失較2024年約4.94億美元下降83%，降至約8,400萬美元，然而同期每位受害者的平均損失卻反而上升——攻擊者已從大規模、低價值的垃圾式攻擊，轉向如本案這種精準鎖定的「獵鯨行動」。部分資安研究人員將這類事件歸類為使用者疏失，而非協議失敗，並主張定期撤銷(revoke)過期核准，或使用具備清晰簽署(clear-signing)功能的硬體錢包，本可避免此事。然而這種觀點可能低估了如今的釣魚介面模仿正規介面的逼真程度。</p>
...
<h2 class="id">Mengapa Ini Penting, dan Apa yang Perlu Diamati</h2>
<h2 class="pl">Dlaczego To Ma Znaczenie i Co Obserwować</h2>
<h2 class="it">Perché È Importante e Cosa Osservare</h2>
<h2 class="ru">Почему Это Важно и За Чем Следить</h2>
<h2 class="zh">為何重要，以及該關注什麼</h2>
...
<p class="id">Seiring modal semakin terkonsentrasi di lebih sedikit dompet, setiap phishing yang berhasil menghasilkan keuntungan lebih besar, yang berarti penurunan 83% kerugian agregat yang disoroti ScamSniffer mungkin secara substansial meremehkan risiko yang sebenarnya dihadapi pemegang besar saat ini. Perlu dicatat juga betapa tajamnya angka $84 juta dari ScamSniffer berbeda dari hitungan CertiK sebesar $723 juta dari 248 insiden pada tahun yang sama — kemungkinan bukan karena salah satu angka itu salah, melainkan karena perusahaan keamanan yang berbeda mendefinisikan cakupan "phishing" secara berbeda (hanya drainer dompet, dibandingkan dengan yang juga menghitung eksploitasi bridge dan multisig), yang membuat berisiko untuk membaca satu angka industri secara tunggal sebagai gambaran utuh.</p>
<p class="pl">W miarę jak kapitał coraz bardziej koncentruje się w mniejszej liczbie portfeli, każdy udany phishing przynosi większy zysk, co oznacza, że spadek strat zagregowanych o 83%, który podkreślił ScamSniffer, może znacząco zaniżać ryzyko, z jakim faktycznie mierzą się dziś duzi posiadacze. Warto też zauważyć, jak mocno liczba 84 milionów dolarów ScamSniffer odbiega od liczby CertiK wynoszącej 723 miliony dolarów w 248 incydentach w tym samym roku — prawdopodobnie nie dlatego, że którakolwiek liczba jest błędna, ale dlatego, że różne firmy zajmujące się bezpieczeństwem inaczej definiują zakres „phishingu" (tylko drenery portfeli, w porównaniu z liczeniem także exploitów mostów i multisig), co sprawia, że ryzykowne jest odczytywanie jakiejkolwiek pojedynczej liczby ogólnobranżowej jako pełnego obrazu.</p>
<p class="it">Man mano che il capitale si concentra sempre più in un numero minore di portafogli, ogni phishing riuscito rende di più, il che significa che il calo dell'83% delle perdite aggregate evidenziato da ScamSniffer potrebbe sottostimare sostanzialmente il rischio che oggi affrontano realmente i grandi detentori. Vale anche la pena notare quanto nettamente la cifra di 84 milioni di dollari di ScamSniffer diverga dal conteggio di CertiK di 723 milioni di dollari su 248 incidenti nello stesso anno — probabilmente non perché uno dei due numeri sia sbagliato, ma perché diverse società di sicurezza definiscono il "phishing" in modo diverso (solo drainer di portafogli, contro il conteggio anche di exploit di bridge e multisig), il che rende rischioso leggere una singola cifra a livello di settore come il quadro completo.</p>
<p class="ru">По мере того как капитал всё больше концентрируется в меньшем числе кошельков, каждый успешный фишинг приносит больше выгоды, а значит снижение совокупных потерь на 83%, отмеченное ScamSniffer, может существенно занижать риск, с которым сегодня реально сталкиваются крупные держатели. Также стоит отметить, насколько резко цифра ScamSniffer в 84 миллиона долларов расходится с подсчётом CertiK в 723 миллиона долларов по 248 инцидентам за тот же год — вероятно, не потому, что какая-то из цифр неверна, а потому что разные компании в сфере безопасности по-разному определяют границы понятия «фишинг» (только дренеры кошельков против также учёта эксплойтов мостов и мультиподписи), из-за чего рискованно воспринимать любую отдельную отраслевую цифру как полную картину.</p>
<p class="zh">隨著資金日益集中於少數錢包，每一次成功的釣魚攻擊所帶來的收益也隨之增加，這意味著ScamSniffer所強調的整體損失下降83%，可能大幅低估了大戶如今實際面臨的風險。同樣值得注意的是，ScamSniffer的8,400萬美元數字，與CertiK同年統計的248起事件、7.23億美元損失之間存在巨大落差——這很可能不是因為哪個數字有誤，而是因為不同資安公司對「網路釣魚」的定義範圍不同（僅計入錢包抽乾攻擊，或同時計入跨鏈橋與多重簽章漏洞攻擊），這使得單憑任何一個產業層級的數字來判斷全貌都存在風險。</p>
...
<p class="id">Ada dua hal yang patut diamati ke depan. Pertama, seberapa cepat dompet-dompet utama mengadopsi default persetujuan yang kedaluwarsa dan alat pencabutan seperti Revoke.cash. Kedua, apakah standar abstraksi akun baru seperti EIP-7702 memperbaiki atau memperburuk permukaan serangan phishing — para peneliti telah menerbitkan serangan phishing bukti konsep (PoC) EIP-7702, menunjukkan bahwa generasi berikutnya dompet abstraksi akun dapat kembali memunculkan kerentanan berbasis tanda tangan serupa jika tidak dirancang dengan cermat.</p>
<p class="pl">Na przyszłość warto obserwować dwie rzeczy. Po pierwsze, jak szybko główne portfele przyjmą domyślnie wygasające zatwierdzenia oraz narzędzia do odwoływania, takie jak Revoke.cash. Po drugie, czy nowe standardy abstrakcji konta, takie jak EIP-7702, poprawią, czy pogorszą powierzchnię ataku phishingowego — badacze opublikowali już ataki phishingowe typu proof-of-concept na EIP-7702, co sugeruje, że kolejna generacja portfeli z abstrakcją konta może ponownie wprowadzić podobną lukę opartą na podpisach, jeśli nie zostanie starannie zaprojektowana.</p>
<p class="it">Due cose meritano attenzione in futuro. Primo, quanto velocemente i principali portafogli adotteranno approvazioni predefinite con scadenza e strumenti di revoca come Revoke.cash. Secondo, se i nuovi standard di account abstraction come EIP-7702 miglioreranno o peggioreranno la superficie di attacco del phishing — i ricercatori hanno già pubblicato attacchi di phishing proof-of-concept su EIP-7702, suggerendo che la prossima generazione di portafogli con account abstraction potrebbe reintrodurre una vulnerabilità simile basata su firme se non progettata con attenzione.</p>
<p class="ru">В будущем стоит следить за двумя вещами. Во-первых, насколько быстро крупные кошельки внедрят истекающие по умолчанию одобрения и инструменты отзыва, такие как Revoke.cash. Во-вторых, улучшат или ухудшат новые стандарты абстракции аккаунта, такие как EIP-7702, поверхность фишинговых атак — исследователи уже опубликовали proof-of-concept фишинговые атаки на EIP-7702, что говорит о том, что следующее поколение кошельков с абстракцией аккаунта может вновь привнести похожую уязвимость на основе подписей, если не будет тщательно спроектировано.</p>
<p class="zh">未來有兩件事值得關注。首先，主要錢包採用具有到期時限的核准預設值，以及Revoke.cash等撤銷工具的速度有多快。其次，像EIP-7702這類新的帳戶抽象化(account abstraction)標準，究竟會改善還是惡化網路釣魚的攻擊面——研究人員已發表針對EIP-7702的概念驗證(PoC)釣魚攻擊，顯示若設計不夠審慎，下一代帳戶抽象化錢包可能會重新引入類似的、以簽名為基礎的漏洞。</p>
...
<div class="id">
<svg viewBox="0 0 700 420" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;background:#111113;border-radius:12px;border:1px solid rgba(255,255,255,.08);margin:20px 0">
  <text x="20" y="28" fill="#fafafa" font-size="16" font-weight="700" font-family="sans-serif">Satu Tanda Tangan → 36 Detik → $999.999 USDT Hilang</text>
  <g font-family="sans-serif">
    <rect x="20" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="165" y="80" fill="#e4e4e7" font-size="13" font-weight="700" text-anchor="middle">① Persetujuan berbahaya ditandatangani</text>
    <text x="165" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">Tanpa gas · belum on-chain</text>
    <text x="165" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">→ Terlihat seperti "tidak terjadi apa-apa"</text>

    <rect x="390" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#f87171" stroke-width="1"/>
    <text x="535" y="80" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">② Percobaan penguras penuh (gagal)</text>
    <text x="535" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">~$1 juta diminta → dana tidak cukup</text>
    <text x="535" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">Skrip mendeteksi penolakan otomatis</text>

    <line x1="165" y1="135" x2="165" y2="165" stroke="#3f3f46" stroke-width="2"/>
    <line x1="535" y1="135" x2="535" y2="165" stroke="#f87171" stroke-width="2"/>
    <text x="350" y="158" fill="#fbbf24" font-size="14" font-weight="700" text-anchor="middle">Menghitung ulang 36 detik kemudian</text>

    <rect x="20" y="175" width="660" height="95" rx="10" fill="#18181b" stroke="#71717a" stroke-width="1"/>
    <text x="350" y="200" fill="#fafafa" font-size="14" font-weight="700" text-anchor="middle">③ 3 penarikan digabung dalam satu Multicall</text>
    <text x="180" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">639.999 USDT</text>
    <text x="350" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">159.999 USDT</text>
    <text x="520" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">200.000 USDT</text>
    <text x="350" y="252" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">Total ≈ 999.998 USDT ($999.999)</text>
    ...
    <rect x="20" y="285" width="660" height="105" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="40" y="311" fill="#fafafa" font-size="14" font-weight="700">Data ScamSniffer 2025</text>
    <text x="40" y="335" fill="#e4e4e7" font-size="13">Total kerugian phishing: $494 juta (2024) → $84 juta (2025), -83%</text>
    <text x="40" y="357" fill="#e4e4e7" font-size="13">Kerugian rata-rata per korban justru meningkat — serangan "perburuan paus" menyebar</text>
    <text x="40" y="378" fill="#a1a1aa" font-size="12">Selisih dengan hitungan CertiK (248 insiden, $723 juta di 2025) berasal dari perbedaan cakupan "phishing"</text>
  </g>
  <text x="350" y="408" fill="#71717a" font-size="12" text-anchor="middle" font-family="sans-serif">Sumber: Cointelegraph, CryptoTimes, crypto.news, ScamSniffer, CertiK · per Juli 2026</text>
</svg>
</div>
<div class="pl">
<svg viewBox="0 0 700 420" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;background:#111113;border-radius:12px;border:1px solid rgba(255,255,255,.08);margin:20px 0">
  <text x="20" y="28" fill="#fafafa" font-size="16" font-weight="700" font-family="sans-serif">Jeden Podpis → 36 Sekund → 999 999 $ USDT Zniknęło</text>
  <g font-family="sans-serif">
    <rect x="20" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="165" y="80" fill="#e4e4e7" font-size="13" font-weight="700" text-anchor="middle">① Podpisano złośliwe zatwierdzenie</text>
    <text x="165" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">Bez gazu · jeszcze nie on-chain</text>
    <text x="165" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">→ Wygląda jak „nic się nie stało"</text>

    <rect x="390" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#f87171" stroke-width="1"/>
    <text x="535" y="80" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">② Próba pełnego drenażu (nieudana)</text>
    <text x="535" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">żądano ~1 mln $ → za mało środków</text>
    <text x="535" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">Skrypt automatycznie wykrywa odrzucenie</text>

    <line x1="165" y1="135" x2="165" y2="165" stroke="#3f3f46" stroke-width="2"/>
    <line x1="535" y1="135" x2="535" y2="165" stroke="#f87171" stroke-width="2"/>
    <text x="350" y="158" fill="#fbbf24" font-size="14" font-weight="700" text-anchor="middle">Ponowne przeliczenie po 36 sek.</text>

    <rect x="20" y="175" width="660" height="95" rx="10" fill="#18181b" stroke="#71717a" stroke-width="1"/>
    <text x="350" y="200" fill="#fafafa" font-size="14" font-weight="700" text-anchor="middle">③ 3 wypłaty połączone w jednym Multicall</text>
    <text x="180" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">639 999 USDT</text>
    <text x="350" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">159 999 USDT</text>
    <text x="520" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">200 000 USDT</text>
    <text x="350" y="252" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">Suma ≈ 999 998 USDT (999 999 $)</text>

    <rect x="20" y="285" width="660" height="105" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="40" y="311" fill="#fafafa" font-size="14" font-weight="700">Dane ScamSniffer za 2025 rok</text>
    <text x="40" y="335" fill="#e4e4e7" font-size="13">Łączne straty z phishingu: 494 mln $ (2024) → 84 mln $ (2025), -83%</text>
    <text x="40" y="357" fill="#e4e4e7" font-size="13">Średnia strata na ofiarę wzrosła — rozprzestrzenia się "polowanie na wieloryby"</text>
    <text x="40" y="378" fill="#a1a1aa" font-size="12">Różnica względem CertiK (248 incydentów, 723 mln $ w 2025) wynika z innego zakresu "phishingu"</text>
  </g>
  <text x="350" y="408" fill="#71717a" font-size="12" text-anchor="middle" font-family="sans-serif">Źródła: Cointelegraph, CryptoTimes, crypto.news, ScamSniffer, CertiK · stan na lipiec 2026</text>
</svg>
</div>
<div class="it">
<svg viewBox="0 0 700 420" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;background:#111113;border-radius:12px;border:1px solid rgba(255,255,255,.08);margin:20px 0">
  <text x="20" y="28" fill="#fafafa" font-size="16" font-weight="700" font-family="sans-serif">Una Firma → 36 Secondi → 999.999 $ USDT Spariti</text>
  <g font-family="sans-serif">
    <rect x="20" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="165" y="80" fill="#e4e4e7" font-size="13" font-weight="700" text-anchor="middle">① Approvazione malevola firmata</text>
    <text x="165" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">Nessun gas · non ancora on-chain</text>
    <text x="165" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">→ Sembra che "non sia successo nulla"</text>

    <rect x="390" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#f87171" stroke-width="1"/>
    <text x="535" y="80" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">② Tentativo di prosciugamento totale (fallisce)</text>
    <text x="535" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">~1 mln $ richiesti → fondi insufficienti</text>
    <text x="535" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">Lo script rileva automaticamente il rifiuto</text>

    <line x1="165" y1="135" x2="165" y2="165" stroke="#3f3f46" stroke-width="2"/>
    <line x1="535" y1="135" x2="535" y2="165" stroke="#f87171" stroke-width="2"/>
    <text x="350" y="158" fill="#fbbf24" font-size="14" font-weight="700" text-anchor="middle">Ricalcola dopo 36 secondi</text>

    <rect x="20" y="175" width="660" height="95" rx="10" fill="#18181b" stroke="#71717a" stroke-width="1"/>
    <text x="350" y="200" fill="#fafafa" font-size="14" font-weight="700" text-anchor="middle">③ 3 prelievi raggruppati in un unico Multicall</text>
    <text x="180" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">639.999 USDT</text>
    <text x="350" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">159.999 USDT</text>
    <text x="520" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">200.000 USDT</text>
    <text x="350" y="252" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">Totale ≈ 999.998 USDT (999.999 $)</text>

    <rect x="20" y="285" width="660" height="105" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="40" y="311" fill="#fafafa" font-size="14" font-weight="700">Dati ScamSniffer 2025</text>
    <text x="40" y="335" fill="#e4e4e7" font-size="13">Perdite totali da phishing: 494 mln $ (2024) → 84 mln $ (2025), -83%</text>
    <text x="40" y="357" fill="#e4e4e7" font-size="13">La perdita media per vittima è invece aumentata — si diffonde la "caccia alle balene"</text>
    <text x="40" y="378" fill="#a1a1aa" font-size="12">Il divario col conteggio di CertiK (248 incidenti, 723 mln $ nel 2025) deriva dal diverso ambito di "phishing"</text>
  </g>
  <text x="350" y="408" fill="#71717a" font-size="12" text-anchor="middle" font-family="sans-serif">Fonti: Cointelegraph, CryptoTimes, crypto.news, ScamSniffer, CertiK · aggiornato a luglio 2026</text>
</svg>
</div>
<div class="ru">
<svg viewBox="0 0 700 420" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;background:#111113;border-radius:12px;border:1px solid rgba(255,255,255,.08);margin:20px 0">
  <text x="20" y="28" fill="#fafafa" font-size="16" font-weight="700" font-family="sans-serif">Одна Подпись → 36 Секунд → 999 999 $ USDT Исчезли</text>
  <g font-family="sans-serif">
    <rect x="20" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="165" y="80" fill="#e4e4e7" font-size="13" font-weight="700" text-anchor="middle">① Подписано вредоносное одобрение</text>
    <text x="165" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">Без газа · ещё не в блокчейне</text>
    <text x="165" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">→ Выглядит, будто «ничего не произошло»</text>

    <rect x="390" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#f87171" stroke-width="1"/>
    <text x="535" y="80" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">② Попытка полного вывода (неудача)</text>
    <text x="535" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">запрошено ~1 млн $ → недостаточно средств</text>
    <text x="535" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">Скрипт автоматически фиксирует отказ</text>

    <line x1="165" y1="135" x2="165" y2="165" stroke="#3f3f46" stroke-width="2"/>
    <line x1="535" y1="135" x2="535" y2="165" stroke="#f87171" stroke-width="2"/>
    <text x="350" y="158" fill="#fbbf24" font-size="14" font-weight="700" text-anchor="middle">Пересчёт через 36 секунд</text>

    <rect x="20" y="175" width="660" height="95" rx="10" fill="#18181b" stroke="#71717a" stroke-width="1"/>
    <text x="350" y="200" fill="#fafafa" font-size="14" font-weight="700" text-anchor="middle">③ 3 вывода объединены в один Multicall</text>
    <text x="180" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">639 999 USDT</text>
    <text x="350" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">159 999 USDT</text>
    <text x="520" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">200 000 USDT</text>
    <text x="350" y="252" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">Итого ≈ 999 998 USDT (999 999 $)</text>

    <rect x="20" y="285" width="660" height="105" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="40" y="311" fill="#fafafa" font-size="14" font-weight="700">Данные ScamSniffer за 2025 год</text>
    <text x="40" y="335" fill="#e4e4e7" font-size="13">Общие потери от фишинга: 494 млн $ (2024) → 84 млн $ (2025), -83%</text>
    <text x="40" y="357" fill="#e4e4e7" font-size="13">Средний убыток на жертву, наоборот, вырос — распространяется «охота на китов»</text>
    <text x="40" y="378" fill="#a1a1aa" font-size="12">Разрыв с подсчётом CertiK (248 инцидентов, 723 млн $ в 2025) связан с разным охватом понятия «фишинг»</text>
  </g>
  <text x="350" y="408" fill="#71717a" font-size="12" text-anchor="middle" font-family="sans-serif">Источники: Cointelegraph, CryptoTimes, crypto.news, ScamSniffer, CertiK · по состоянию на июль 2026</text>
</svg>
</div>
<div class="zh">
<svg viewBox="0 0 700 420" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;background:#111113;border-radius:12px;border:1px solid rgba(255,255,255,.08);margin:20px 0">
  <text x="20" y="28" fill="#fafafa" font-size="16" font-weight="700" font-family="sans-serif">一次簽名 → 36秒 → 999,999美元USDT消失</text>
  <g font-family="sans-serif">
    <rect x="20" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="165" y="80" fill="#e4e4e7" font-size="13" font-weight="700" text-anchor="middle">① 簽署惡意核准</text>
    <text x="165" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">無需Gas費・尚未上鏈</text>
    <text x="165" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">→ 看似「什麼都沒發生」</text>

    <rect x="390" y="55" width="290" height="80" rx="10" fill="#18181b" stroke="#f87171" stroke-width="1"/>
    <text x="535" y="80" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">② 全額抽乾嘗試(失敗)</text>
    <text x="535" y="102" fill="#a1a1aa" font-size="12" text-anchor="middle">要求約100萬美元 → 餘額不足</text>
    <text x="535" y="120" fill="#a1a1aa" font-size="12" text-anchor="middle">腳本自動偵測到遭拒</text>

    <line x1="165" y1="135" x2="165" y2="165" stroke="#3f3f46" stroke-width="2"/>
    <line x1="535" y1="135" x2="535" y2="165" stroke="#f87171" stroke-width="2"/>
    <text x="350" y="158" fill="#fbbf24" font-size="14" font-weight="700" text-anchor="middle">36秒後重新計算</text>

    <rect x="20" y="175" width="660" height="95" rx="10" fill="#18181b" stroke="#71717a" stroke-width="1"/>
    <text x="350" y="200" fill="#fafafa" font-size="14" font-weight="700" text-anchor="middle">③ 3筆提領合併於單一Multicall</text>
    <text x="180" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">639,999 USDT</text>
    <text x="350" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">159,999 USDT</text>
    <text x="520" y="228" fill="#e4e4e7" font-size="13" text-anchor="middle">200,000 USDT</text>
    <text x="350" y="252" fill="#fca5a5" font-size="13" font-weight="700" text-anchor="middle">總計 ≈ 999,998 USDT($999,999)</text>

    <rect x="20" y="285" width="660" height="105" rx="10" fill="#18181b" stroke="#3f3f46" stroke-width="1"/>
    <text x="40" y="311" fill="#fafafa" font-size="14" font-weight="700">ScamSniffer 2025年資料</text>
    <text x="40" y="335" fill="#e4e4e7" font-size="13">網路釣魚總損失：2024年4.94億美元 → 2025年0.84億美元(-83%)</text>
    <text x="40" y="357" fill="#e4e4e7" font-size="13">每位受害者平均損失反而上升 —「獵鯨」式精準攻擊擴散</text>
    <text x="40" y="378" fill="#a1a1aa" font-size="12">與CertiK統計(2025年248起、7.23億美元)的落差源於「網路釣魚」定義範圍不同</text>
  </g>
  <text x="350" y="408" fill="#71717a" font-size="12" text-anchor="middle" font-family="sans-serif">資料來源：Cointelegraph、CryptoTimes、crypto.news、ScamSniffer、CertiK · 截至2026年7月</text>
</svg>
</div>
...
<div class="box id">💡 <strong>Kesimpulan:</strong> Pada 9 Juli, sebuah dompet Ethereum kehilangan 999.999 USDT akibat satu tanda tangan persetujuan token berbahaya. Skrip otomatis gagal menguras seluruh saldo, lalu menghitung ulang 36 detik kemudian dan menarik dana dalam tiga penarikan yang digabungkan melalui Multicall. Ini bukan bug protokol — ini adalah kelemahan struktural dalam cara approve tanpa batas dan permit EIP-2612 dirancang secara default, dan seperti yang ditunjukkan data ScamSniffer, industri ini bergeser dari spam massal bernilai rendah menuju "perburuan paus" yang menargetkan lebih sedikit dompet, namun lebih kaya.</div>
<div class="box pl">💡 <strong>Podsumowanie:</strong> 9 lipca portfel Ethereum stracił 999 999 USDT w wyniku jednego złośliwego podpisu zatwierdzenia tokena. Zautomatyzowany skrypt nie zdołał zdrenować całego salda, następnie przeliczył ponownie 36 sekund później i wyprowadził środki w trzech wypłatach połączonych przez Multicall. To nie jest błąd protokołu — to strukturalna słabość w tym, jak domyślnie projektowane są nieograniczone approve i permit EIP-2612, a jak pokazują dane ScamSniffer, branża przechodzi od masowego, niskowartościowego spamu do „polowania na wieloryby", celującego w mniej, ale zamożniejszych posiadaczy portfeli.</div>
<div class="box it">💡 <strong>In sintesi:</strong> Il 9 luglio, un portafoglio Ethereum ha perso 999.999 USDT a causa di una singola firma malevola di approvazione token. Uno script automatizzato non è riuscito a prosciugare l'intero saldo, poi ha ricalcolato 36 secondi dopo e prelevato i fondi in tre prelievi raggruppati tramite Multicall. Non è un bug del protocollo — è una debolezza strutturale nel modo in cui l'approve illimitato e il permit EIP-2612 sono progettati per impostazione predefinita, e come mostrano i dati di ScamSniffer, il settore sta passando dallo spam di massa a basso valore alla "caccia alle balene" mirata a un numero minore di portafogli, ma più ricchi.</div>
<div class="box ru">💡 <strong>Итог:</strong> 9 июля кошелёк Ethereum потерял 999 999 USDT из-за одной вредоносной подписи одобрения токена. Автоматизированный скрипт не смог вывести весь баланс сразу, затем через 36 секунд пересчитал сумму и вывел средства тремя выводами, объединёнными через Multicall. Это не баг протокола — это структурная слабость в том, как по умолчанию спроектированы неограниченный approve и permit EIP-2612, и, как показывают данные ScamSniffer, отрасль переходит от массового низкоценного спама к «охоте на китов», нацеленной на меньшее число, но более состоятельных держателей кошельков.</div>
<div class="box zh">💡 <strong>重點整理：</strong>7月9日，一個以太坊錢包因單次惡意代幣核准簽名損失了999,999 USDT。自動化腳本未能一次抽乾全部餘額，隨後於36秒後重新計算，透過三筆以Multicall合併的提領取走資金。這並非協議漏洞——而是無限額approve與EIP-2612 permit預設設計上的結構性弱點，而正如ScamSniffer的資料所示，這個產業正從大規模、低價值的垃圾攻擊，轉向鎖定少數但更富有錢包的「獵鯨行動」。</div>
...
<p class="id" style="font-size:12px;color:#52525b;margin-top:24px">Sumber: Cointelegraph, CryptoTimes, crypto.news, GNCrypto, Cryip, UseTheBitcoin, ScamSniffer, CertiK. Angka kerugian dan detail transaksi sesuai dengan waktu pemberitaan masing-masing media dan dapat berubah seiring berlanjutnya investigasi.</p>
<p class="pl" style="font-size:12px;color:#52525b;margin-top:24px">Źródła: Cointelegraph, CryptoTimes, crypto.news, GNCrypto, Cryip, UseTheBitcoin, ScamSniffer, CertiK. Kwoty strat i szczegóły transakcji są zgodne ze stanem na moment publikacji przez poszczególne redakcje i mogą ulec zmianie w miarę postępu dochodzeń.</p>
<p class="it" style="font-size:12px;color:#52525b;margin-top:24px">Fonti: Cointelegraph, CryptoTimes, crypto.news, GNCrypto, Cryip, UseTheBitcoin, ScamSniffer, CertiK. Le cifre delle perdite e i dettagli delle transazioni sono aggiornati al momento della pubblicazione di ciascuna testata e potrebbero cambiare con il proseguire delle indagini.</p>
<p class="ru" style="font-size:12px;color:#52525b;margin-top:24px">Источники: Cointelegraph, CryptoTimes, crypto.news, GNCrypto, Cryip, UseTheBitcoin, ScamSniffer, CertiK. Цифры потерь и детали транзакций приведены на момент публикации каждого издания и могут измениться по мере продолжения расследований.</p>
<p class="zh" style="font-size:12px;color:#52525b;margin-top:24px">資料來源：Cointelegraph、CryptoTimes、crypto.news、GNCrypto、Cryip、UseTheBitcoin、ScamSniffer、CertiK。損失金額與交易細節以各媒體報導當下為準，並可能隨調查進展而變動。</p>
